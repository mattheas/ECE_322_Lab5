...
# create system
    ...
    def test_run_load_command_method(self):
        # test load command
        with open('./tests/TestABCDEFG_redirect_output_load_command_actual.txt', 'w') as f1:
            with redirect_stdout(f1):
        ...
        # assert contents of expected file is the same as actual
        f1 = open("./tests/TestABCDEFG_redirect_output_load_command_actual.txt", "r")
        f2 = open("./tests/TestABCDEFG_redirect_output_load_command_expected.txt", "r")
        i = 0
        for line1 in f1:
            i+=1
            for line2 in f2:
